...
# 		img = self.segment_image(img)
		...
		erosion_kernel = np.ones((15,15),np.uint8)   # erosion kernel
		img = cv2.erode(img,erosion_kernel,iterations = 1)  # erode image
		dilate_kernel = np.ones((5,5), np.uint8)  # dilate kernel
...
# 			print('loop through contours_sorted',contour)
			...
			aspect_ratio = temp_box[2] / float(temp_box[3])
